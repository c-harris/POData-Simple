...
abstract class LaravelQueryProvider implements IQueryProvider
{
    /**
    * @var Connection
    */
    ...
    {
        return new PHPExpressionProvider(null);
    }
    /**
...
//                $result .= $order->isAscending() ? ' ASC' : ' DESC';
	 ...
	 protected function getOrderByExpressionAsArray(InternalOrderByInfo $orderBy)
    {
        ...
        foreach ($orderBy->getOrderByInfo()->getOrderByPathSegments() as $order) {
            foreach ($order->getSubPathSegments() as $subOrder) {
				$result[] = array($subOrder->getName(),$order->isAscending() ? 'asc' : 'desc');
    ...
    public function getResourceSet(
		...
		$results;
		if($filterInfo){
			$results = $entityClassName::where($filterInfo->getExpressionAsString());
		}else{
			$results = $entityClassName::all();
		}
		if($orderBy){
			$orderByArray = getOrderByExpressionAsArray($orderBy);
			foreach($orderByArray as $o){
    			$results = $results->orderby($o[0],$o[1]);
			}
		}
        $sqlCount = count($results);
        if ($queryType == QueryType::ENTITIES() || $queryType == QueryType::ENTITIES_WITH_COUNT()) {
			$data = $results;
            if ($queryType == QueryType::ENTITIES_WITH_COUNT()){
				                $result->count = $this->queryScalar($sqlCount);
			}
			// $result->results = array_map($entityClassName . '::fromRecord', $data); #I added this to a trait.... don't remember why. I think it is for metadata mapping
			$result->results = $data;
		}
